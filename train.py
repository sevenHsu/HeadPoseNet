...
def parse_args():
    ...
    parser.add_argument('--train_data', dest='train_data', help='Directory path for data.',
                        default='/home/300WLPQUAT', type=str)
    parser.add_argument('--valid_data', dest='valid_data', help='Directory path for data.',
                        default='/home/AFLW2000QUAT', type=str)
    parser.add_argument('--net', dest='net', help='net name for training',
                        choices=['resnet', 'mobilenet', 'shufflenet', 'squeezenet', 'squeezenet_bn'],
                        default='net', type=str)
    parser.add_argument('--save_dir', dest='save_dir', help='model saving path',
                        default='/train/trainset/1/HeadPoseNetResults', type=str)
    parser.add_argument('--snapshot', dest='snapshot', help='pretrained wight path',
                        default='/home/net_face.pth', type=str)
...
def validation_reg(model, valid_loader):
    ...
    with torch.no_grad():
        ...
        for j, (valid_img, valid_vector_label, _, _, _) in enumerate(valid_loader):
            ...
            cos_value_front = vector_cos(pred_valid_vector[:3], valid_vector_label[:3])
            degrees_error += torch.mean(torch.acos(cos_value_front) * 180 / np.pi)

            cos_value_top = vector_cos(pred_valid_vector[3:], valid_vector_label[3:])
            degrees_error += torch.mean(torch.acos(cos_value_top) * 180 / np.pi)
...
def train_reg(model, train_loader, valid_loader, reg_criterion, output_string, args):
    ...
    for epoch in range(num_epochs):
        ...
        for i, (images, label_vector, name) in enumerate(train_loader):
            ...
            loss = reg_criterion(pred_vector[:, :3], label_vector[:, :3]) + reg_criterion(pred_vector[:, 3:], label_vector[:, 3:])

            # front vector
            cos_value_front = utils.vector_cos(pred_vector[:, :3], label_vector[:, :3])
            degrees_error_front = torch.mean(torch.acos(cos_value_front) * 180 / np.pi)

            # top vector
            cos_value_top = utils.vector_cos(pred_vector[:, 3:], label_vector[:, 3:])
            degrees_error_top = torch.mean(torch.acos(cos_value_top) * 180 / np.pi)
            ...
            if (i + 1) % 100 == 0:
                msg = 'Epoch [%d/%d], Iter [%d/%d] mse_loss: %.4f degrees_error_front:%.3f degrees_error_top:%.3f' % (
                    epoch + 1, num_epochs, i + 1, len(train_dataset) // batch_size, loss.item(), degrees_error_front.item(), degrees_error_top.item())
...
if __name__ == "__main__":
    args = parse_args()
    if args.mode == 'reg':
        output_string = args.net + '_reg_width_mult%s_%s' % (str(args.width_mult).replace('.', '_'),
                                                             str(args.input_size))
    else:
        if args.top_k is None:
            output_string = args.net + "_%s_%s_top%s_alpha%s_width_mult%s_%s" % (args.mode,
                                                                                 args.num_classes,
                                                                                 args.top_k,
            ...
            output_string = args.net + "_%s_%s_top%s_%s_alpha%s_width_mult%s_%s" % (args.mode,
    ...
    if args.mode in ['cls', 'sord']:
        ...
        elif args.net == 'squeezenet_bn':
            model = SqueezeNetBn(num_classes=args.num_classes)
    ...
    # reg
        ...
        if args.net == 'resnet':
            ...
            model = SqueezeNetReg(version=args.squeeze_version)
            if args.squeeze_version == '1_0':
                load_filtered_state_dict(model, model_zoo.load_url(squeezenet_model_urls['squeezenet1_0']))
            elif args.squeeze_version == '1_1':
                load_filtered_state_dict(model, model_zoo.load_url(squeezenet_model_urls['squeezenet1_1']))
